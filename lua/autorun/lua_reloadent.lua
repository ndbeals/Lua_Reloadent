--------------------------------------------------------------
-- Original Creater: TomyLobo                               --
-- Updated By: TGiFallen                                    --
-- Usage:                                                   --
...
local function luaExists(luaname)
	return file.Exists( luaname , "LUA" )
end
...
local function lua_reloadtool(toolmode)
	...
	SWEP = gmod_tool
	TOOL = metatable 
	ToolObj = getmetatable(metatable)
	--TOOL = ToolObj:Create()
	TOOL.Mode = toolmode

	if metatable.SourceFile then 
		include(metatable.SourceFile)
...
local function lua_reloadwep(entname, filename)
	...
	if entname == "gmod_tool" or luaExists(metatable.Folder..filename) then
		include(metatable.Folder..filename)
	elseif luaExists(metatable.Folder.."/shared.lua") then
		include(metatable.Folder.."/shared.lua")
	elseif luaExists(metatable.Folder..".lua") then
		include(metatable.Folder..".lua")
	else
